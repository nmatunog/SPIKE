...
import { Week5Day1MissionFlow } from './Week5Day1MissionFlow.jsx';
 ...
 */
export function Week5Day1MissionPlaybookView({
  bundle,
  participantId,
  programWeek = 5,
  ...
  const deck = bundle ? resolvePresentations(bundle, ['presentation-w5-d1-deck-01'])[0] : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <Week5PlaybookHero allowDownload={staffPreview} showDay1Deck />
      {deck?.slides?.length ? (
      ...
      <section className="rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50/60 p-4 shadow-card sm:p-6">
        <div className="mb-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-spike">Week 5 · Day 1</p>
          <h2 className="mt-1 text-xl font-bold text-slate-900 sm:text-2xl">Finalize the Business</h2>
          <p className="mt-2 max-w-2xl text-sm text-slate-600">
            A convincing pitch begins with a business that makes sense. Review, challenge assumptions, and strengthen weak points — save anytime, even with incomplete fields.
          </p>
        </div>
        <Week5Day1MissionFlow participantId={participantId} readOnly={staffPreview} onProgress={onProgress} />
      </section>
      {onOpenCurriculum ? (
