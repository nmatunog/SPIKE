...
export default function GameStatusBar({
  ...
  if (age == null && lifeScore == null) return null

  const bandClass = BAND_COLORS[financialHealth?.band] ?? BAND_COLORS.stable

  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 rounded-lg bg-slate-900 px-4 py-2 text-xs text-slate-100 shadow-sm">
      {age != null && (
