 ...
 * @param {{
 *   verifyActor: (env: Record<string, string>, request: Request) => Promise<object | null>,
 *   createService: (env: Record<string, string>) => import('@supabase/supabase-js').SupabaseClient,
 *   getAnonKey: (env: Record<string, string>) => string,
 *   missingAnonMessage?: string,
 *   internProgramSlug?: string | null,
 * }} deps
 */
export async function handlePortalUsersAdmin(ctx, deps) {
  ...
  const anonKey = deps.getAnonKey(env);
  if (!anonKey) {
    return json(
      {
        message:
          deps.missingAnonMessage ||
  ...
  const actor = await deps.verifyActor(env, request);
  if (!actor) {
    return json(
      {
        message:
  ...
  try {
    if (action === 'create') {
      ...
      if (role === 'INTERN') {
        ...
        if (deps.internProgramSlug) {
          progressRow.program_slug = deps.internProgramSlug;
        }
